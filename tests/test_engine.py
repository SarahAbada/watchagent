...
def test_vancouver_persistent_scs_at_minus_six_ottawa_does_not_trigger(
...
def test_toronto_persistent_heat_wave_requires_three_hour_streak(
    isolated_db: tuple[str, database.DatabasePool],
) -> None:
    _, pool = isolated_db
    engine = WeatherEvaluationEngine(pool)
    ...
    heat_events = [event for event in events if event["event_type"] == "EHW"]

    assert len(heat_events) == 1
    assert heat_events[0]["city"] == "Toronto"
    assert "subway traction power strain" in heat_events[0]["rationale"]
    assert "34.0C" in heat_events[0]["rationale"]


def test_single_hour_scs_and_ehw_spikes_are_ignored(
    isolated_db: tuple[str, database.DatabasePool],
) -> None:
    _, pool = isolated_db
    engine = WeatherEvaluationEngine(pool)

    vancouver_cold_spike = _reading(
        city="Vancouver",
        timestamp="2026-05-28T12:00",
        temperature_2m=-6.0,
        apparent_temperature=-8.0,
    ...
    vancouver_events = engine.evaluate_reading(vancouver_cold_spike)
    toronto_events = engine.evaluate_reading(toronto_heat_spike)

    assert "SCS" not in _event_types(vancouver_events)
    assert "EHW" not in _event_types(toronto_events)


def test_flash_freeze_fires_with_exact_rationale_on_wet_rapid_plunge(
    isolated_db: tuple[str, database.DatabasePool],
) -> None:
    _, pool = isolated_db
    engine = WeatherEvaluationEngine(pool)
    ...
    events = engine.evaluate_reading(toronto_reading)
    flash_events = [event for event in events if event["event_type"] == "FLASH_FREEZE"]

    assert len(flash_events) == 1
    assert flash_events[0]["rationale"] == (
        "[FLASH_FREEZE] | Delta: -6.0C drop | Context: Rapid drop below freezing on "
        "wet surfaces; extreme ice glaze risk | Station: Toronto"
    )
